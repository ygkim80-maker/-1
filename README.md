...
1. **전자서명 증빙력**
   - 서명 전 본인확인(사번 + 생년월일)
   - 서명 시각(UTC), IP, User-Agent 기록
   - 문서 내용 + 서명 이미지 + 서명자 + 시각을 SHA-256으로 묶어 위변조 확인용 해시 저장
   - `/certificate/{link_id}` 에서 증빙서(인쇄 → PDF 저장) 확인 가능
   - ⚠️ 본 MVP의 서명은 자체 구현 수준입니다. 대외 제출용으로 법적 효력이 필요하다면
     모두싸인/카카오전자서명 등 검증된 전자서명 API 연동을 권장합니다.

2. **현장 실행 가능성**
   - 배송원은 앱 설치나 로그인 없이 개인별 서명 링크(`/sign/{token}`)로 접속
   - 실제 운영 시 카카오 알림톡으로 링크 발송 예정 (본 MVP의 `/api/remind/{driver_id}`는 stub)
